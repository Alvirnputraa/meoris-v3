...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send verification code to email, then open modal
  ...
  return (
    ...
    {/* Code Modal for signup verification */}
    {codeModalOpen && (
      <div className="fixed inset-0 z-50">
        <div className="absolute inset-0 bg-black/40" aria-hidden="true" onClick={() => setCodeModalOpen(false)} />
        <div className="absolute inset-0 flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-white p-6 shadow-xl">
            <h3 className="font-heading text-xl text-black">Verifikasi Email</h3>
            <p className="mt-1 text-sm text-gray-700">Kami telah mengirim 6 digit kode ke {email}.</p>
            <input
              value={code}
              onChange={(e) => setCode(e.target.value.replace(/[^0-9]/g, '').slice(0,6))}
              placeholder="123456"
              inputMode="numeric"
              className="mt-4 w-full border border-gray-300 rounded-md px-3 py-2 text-black focus:outline-none focus:ring-2 focus:ring-black/40 tracking-widest text-center"
            />
            {codeError ? <p className="mt-2 text-sm text-red-600">{codeError}</p> : null}
            <div className="mt-4 flex items-center justify-end gap-2">
              <button className="px-3 py-2 text-sm" onClick={() => setCodeModalOpen(false)}>Batal</button>
              <button className="bg-black text-white px-4 py-2 text-sm" onClick={handleVerifyCode} disabled={isLoading}>
                Verifikasi
              </button>
            </div>
          </div>
