...
const [selectedProvinceId, setSelectedProvinceId] = useState('')
  ...
  const handleVillageSelect = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (value) {
      ...
      setShipPostal(matched?.postalCode ?? '')
    } else {
      setShipKelurahan('')
      setShipPostal('')
    }
  ...
  useEffect(() => {
    ...
    fetch('https://www.emsifa.com/api-wilayah-indonesia/api/provinces.json', { signal: controller.signal })
      ...
      .catch((err) => {
        ...
        setProvinceError('Tidak dapat memuat daftar provinsi. Silakan coba lagi.')
      })
      .finally(() => {
  ...
  if (!selectedProvinceId) {
    ...
    fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/regencies/${selectedProvinceId}.json`, { signal: controller.signal })
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
      ...
      .catch((err) => {
        ...
        setRegencyError('Tidak dapat memuat kota/kabupaten. Silakan coba lagi.')
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/districts/${selectedRegencyId}.json`, { signal: controller.signal })
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
      ...
      .catch((err) => {
        ...
        setDistrictError('Tidak dapat memuat kecamatan. Silakan coba lagi.')
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetch(`https://www.emsifa.com/api-wilayah-indonesia/api/villages/${selectedDistrictId}.json`, { signal: controller.signal })
      .then((res) => {
        if (!res.ok) throw new Error(`HTTP ${res.status}`)
        return res.json() as Promise<Array<{ id: string; name: string; postal_code?: string }>>
      })
      .then((data) => {
        if (!active) return
      const villages: VillageOption[] = data.map((item) => ({
        id: item.id,
        name: item.name,
        postalCode: (item.postal_code || '').toString().trim(),
      }))
      ...
      .catch((err) => {
        ...
        setVillageError('Tidak dapat memuat kelurahan/desa. Silakan coba lagi.')
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    if (!shipPostal && village.postalCode) {
      setShipPostal(village.postalCode)
    }
  }, [selectedVillageId, villageOptions, shipPostal])
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-5 items-start">
                    ...
                    <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
                      ...
                      <div className="bg-gradient-to-r from-gray-50 to-white p-3.5 border-b border-gray-200">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-3.5">
                            ...
                            <div className="group">
                              ...
                              {regencyError ? (
                                <p className="mt-1 text-xs text-red-600">{regencyError}</p>
                              ) : null}
                            </div>

                            {/* Kecamatan - Without Icon */}
                            <div className="group">
                              <label className="block font-belleza text-xs font-semibold text-gray-700 mb-1.5">
                                Kecamatan
                              </label>
