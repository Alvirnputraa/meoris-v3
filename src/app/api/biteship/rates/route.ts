...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) {
      console.error('[Biteship Rates] API key not configured')
    ...
    console.log('[Biteship Rates] Received request:', {
      destination_postal_code,
      items_count: items.length,
      has_area_id: !!destination_area_id
    })

    if (!destination_postal_code) {
      console.error('[Biteship Rates] Missing destination_postal_code')
      return NextResponse.json(
    ...
    const postalStr = String(destination_postal_code).trim()
    if (!/^\d{5,6}$/.test(postalStr)) {
      console.error('[Biteship Rates] Invalid postal code format:', postalStr)
      return NextResponse.json(
        { success: false, error: 'Invalid postal code format. Must be 5-6 digits.' },
        { status: 400 }
      )
    }
    ...
    const ratesPayload: any = {
      ...
      couriers: 'jnt,jne,sicepat,anteraja', // J&T, JNE, SiCepat, and Anteraja
