...
export default function PaymentFailedByIdPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const { data } = await supabase
          .from('orders')
          .select(`id, payment_method, payment_reference, status, total_amount, order_items(*, produk:produk_id(nama_produk, photo1, harga))`)
  ...
  return (
    <main>
      ...
      {/* Desktop header (fixed) - same as pending */}
      <div className="hidden md:flex fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-200">
        <div className="w-full flex items-center justify-between px-6 md:px-8 lg:px-10 py-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/my-account" aria-label="Akun"><Image src="/images/user.png" alt="User" width={26} height={26} className="w-[26px] h-[26px]" /></Link>
          ...
          <aside className="absolute right-0 top-0 h-full w-80 md:w-96 max-w-[92%] bg-white shadow-2xl p-6 flex flex-col">
            ...
            <div className="pt-4">
              ...
              <div className="mt-4 flex flex-col items-stretch gap-3">
                <Link href="/produk/detail-checkout" className="inline-flex items-center justify-center rounded-none border border-black bg-black text-white px-4 py-2 font-body text-sm hover:opacity-90 transition w-full" onClick={() => setIsCartOpen(false)}>Checkout</Link>
              </div>
            </div>
          </aside>
        </div>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-5 py-4">
          ...
          <div className="flex items-center gap-4">
            <a href="#" aria-label="Cari" onClick={(e) => { e.preventDefault(); setIsSearchOpen(true); }}><Image src="/images/search.png" alt="Search" width={26} height={26} /></a>
            <a href="#" aria-label="Favorit" className="relative" onClick={(e) => { e.preventDefault(); setIsFavOpen(true); }}><Image src="/images/favorit.png" alt="Favorit" width={26} height={26} /><span className="absolute -top-2 -right-2 min-w-4 h-4 px-1 rounded-full bg-black text-white text-[10px] leading-4 text-center">{favoritesCount}</span></a>
            <a href="#" aria-label="Keranjang" className="relative" onClick={(e) => { e.preventDefault(); setIsCartOpen(true); }}><Image src="/images/cart.png" alt="Cart" width={26} height={26} /><span className="absolute -top-2 -right-2 min-w-4 h-4 px-1 rounded-full bg-black text-white text-[10px] leading-4 text-center">{cartCount}</span></a>
            <Link href="/my-account" aria-label="Akun"><Image src="/images/user.png" alt="User" width={26} height={26} /></Link>
          </div>
        </div>
      </div>

      {/* Section: body abu (gray) */}
      <section className="bg-white pt-[76px] pb-16 min-h-[55vh] md:min-h-[60vh]">
        <div className="max-w-2xl mx-auto px-4 md:px-8">
          <div className="bg-white p-6 md:p-8 text-center min-h-[480px] md:min-h-[640px]">
            <div className="mx-auto mb-3 md:mb-4 h-12 w-12 md:h-14 md:w-14 rounded-full bg-red-100 flex items-center justify-center">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 6l12 12M18 6L6 18" stroke="#ef4444" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </div>
            <h1 className="font-heading text-xl md:text-3xl text-black">Pembayaran Gagal</h1>
            <p className="mt-1 md:mt-2 font-body text-gray-700 text-sm md:text-base">Maaf, pembayaran Anda tidak berhasil diproses. Silakan coba lagi atau gunakan metode pembayaran lain.</p>
            ...
            <div className="mt-6 text-left">
              <div className="grid grid-cols-2 gap-x-4 md:gap-x-6 gap-y-2 md:gap-y-3 font-body text-xs md:text-sm">
                ...
                <div className={`font-medium text-right ${displayStatus === 'Dibayar' ? 'text-green-600' : displayStatus === 'Gagal' || displayStatus === 'Dibatalkan' ? 'text-red-600' : 'text-amber-600'}`}>{displayStatus}</div>
              ...
              <div className="mt-5 border-t border-gray-200 pt-4 text-left">
                ...
                {!loading && (
                  ...
                  <p className="text-sm text-gray-600">{loading ? 'Memuat detail pesanan...' : 'Detail pesanan tidak tersedia karena pembayaran belum berhasil.'}</p>
                )}
              </div>
            </div>
            ...
            <div className="mt-5 md:mt-6 flex items-center justify-center gap-3">
              {checkoutUrl ? (
                <a href={checkoutUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-none border border-black bg-black text-white px-4 md:px-5 py-2 text-sm hover:opacity-90">Coba Bayar Lagi</a>
              ) : null}
            ...
            <div className="hidden md:flex flex-col items-center text-center space-y-3">
              ...
              <nav className="font-body text-gray-700 text-xs">
                <ul className="flex flex-wrap justify-center gap-x-6 gap-y-2">
                  <li><a href="/docs/notifikasi" className="hover:text-black hover:underline transition-colors">Notifikasi</a></li>
                  <li><a href="/docs/pengembalian" className="hover:text-black hover:underline transition-colors">Pengembalian</a></li>
                  <li><a href="/docs/syarat&ketentuan" className="hover:text-black hover:underline transition-colors">Syarat & Ketentuan</a></li>
                  <li><a href="/docs/kebijakan-privacy" className="hover:text-black hover:underline transition-colors">Kebijakan Privasi</a></li>
                  <li><a href="/my-account" className="hover:text-black hover:underline transition-colors">Detail Akun</a></li>
                </ul>
              </nav>
            </div>
          </div>
