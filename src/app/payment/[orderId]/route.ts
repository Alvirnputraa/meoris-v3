  ...
  try {
    ...
    if (order) {
      if (isPaid(order.status)) {
        return NextResponse.redirect(new URL(`/payment/${cleanOrderId}/succes`, request.url), 302)
      }
      if (isFailed(order.status)) {
        return NextResponse.redirect(new URL(`/payment/${cleanOrderId}/failed`, request.url), 302)
      }
      return NextResponse.redirect(new URL(`/payment/${cleanOrderId}/pending`, request.url), 302)
    }
