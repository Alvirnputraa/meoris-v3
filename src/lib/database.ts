...
export const userDb = {
  // Create new user - FIXED to use Supabase Auth
  async create(email: string, password: string, nama: string) {
    // Use Supabase Auth signup to create user in auth.users
    // emailRedirectTo set to undefined to prevent Supabase sending confirmation email
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      options: {
        ...
        // This will skip email confirmation if disabled in Supabase dashboard
      }
    })
    ...
    // Also insert into public.users for additional data and compatibility
    try {
      await supabase
        .from('users')
        .insert([{
          ...
          email,
          password, // Store password in public.users for backward compatibility
          nama,
      ...
      // Don't throw error - auth.users insert already succeeded
      // User can still login even if public.users insert fails
    }
...
export const keranjangDb = {
  ...
  async addItem(userId: string, produkId: string, quantity = 1, size?: string) {
    ...
    if (existing) {
      ...
      // Item tidak ditemukan, insert baru
      const { data, error } = await supabase
        .from('keranjang')
        .insert({
          user_id: userId,
...
export const praCheckoutDb = {
  ...
  // Get pra-checkout by SHORT_ID with items (for URL routing)
...
// =============================================
// FLASH SALE CONFIG FUNCTIONS
// =============================================

export const flashSaleConfigDb = {
  // Get active flash sale config
  async getActive() {
    const { data, error } = await supabase
  ...
  // Create new flash sale config
  async create(startTime: string, endTime: string) {
    const { data, error } = await supabase
      .from('flash_sale_config')
      .insert([{ start_time: startTime, end_time: endTime, is_active: true }])
      .select()
      .single()
  ...
  // Deactivate all flash sale configs
  async deactivateAll() {
    const { error } = await supabase
      .from('flash_sale_config')
...
export const shippingHistoryDb = {
  // Get shipping history by order ID
  ...
  // Get latest shipping status for an order
