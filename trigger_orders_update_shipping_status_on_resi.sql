...
-- This trigger sets shipping_status to 'Pesanan sedang dikemas' when a real
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    -- On insert, if resi is already set to a non-placeholder value, mark as being prepared
    IF v_resi <> '' AND v_resi <> v_placeholder AND length(v_resi) >= 6
      ...
      NEW.shipping_status := 'Pesanan sedang dikemas';
    END IF;
    RETURN NEW;
