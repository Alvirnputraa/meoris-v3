...
async function updateDeadline() {
  const orderId = 'bce2120b-f653-4bc2-a364-79fb7b89c5cb';
  const paymentRef = 'DEV-T44456309565YP8DU';

  // Set deadline to 18 Nov 2025 12:04:00 WIB (UTC+7)
  // In UTC that's 18 Nov 2025 05:04:00
  const newDeadline = '2025-11-18 12:04:00+07'; // WIB timezone
  const newExpiredTime = Math.floor(new Date('2025-11-18T05:04:00Z').getTime() / 1000);
  ...
  console.log('New Deadline (WIB):', '18 November 2025 12:04:00');
  console.log('New Deadline (UTC):', new Date('2025-11-18T05:04:00Z').toISOString());
  console.log('='.repeat(80));
  ...
  const { data: after, error: afterError } = await supabase
    .from('checkout_submissions')
    .select('payment_expired_at, payment_details, order_summary, status')
    .eq('id', orderId)
    .single();
  ...
  console.log('   Current status:', after.status);
  ...
  console.log('='.repeat(80));
  console.log('Deadline (WIB):', '18 November 2025 12:04:00');
  ...
  console.log('='.repeat(80));
  console.log('\n1. Wait until after 12:04 WIB (deadline passes)');
  ...
  console.log('     "ordersCancelled": 1 (or more),');
  console.log('     "timestamp": "..."');
  console.log('   }');
  console.log('\n4. Or wait for automatic cron (runs every 10 minutes)');
  console.log('   Next cron runs: 12:10, 12:20, 12:30, etc.');
  console.log('='.repeat(80));
}
